...
## v1.76.0.0 （2026-07-17）

### 新增

- 新增 `标注界面()` 在截图上绘制元素矩形和索引编号，返回标注后的 PNG 字节集
- 新增 `OCR识别()` 调用 Windows 内置 OCR 引擎识别图片文字，返回 JSON 文本；需 Windows 10 及以上版本，且系统已安装对应语言的 OCR 包（设置 → 语言和区域 → 添加语言时勾选“OCR”）

### 优化

- 优化 `安卓控件信息` 数据类型，追加 `中心X`/`中心Y`/`宽度`/`高度`/`类短名`/`名称` 6 个派生字段，由 `枚举安卓控件()` 等命令自动填充
- 优化 `雷神adb操作.取屏幕控件()` 增加 `--compressed` 参数压缩输出
- 优化 `雷神adb操作.寻找安卓控件()` 删除死代码，索引参数改为可空
- 优化 `雷神adb操作.枚举安卓控件()` 与 `寻找安卓控件()` 在屏幕控件数量较多时的稳定性，避免出现“分配8字节内存失败”
- 优化 `雷神adb操作.枚举安卓控件()` 与 `寻找安卓控件()` 返回的标题、备注自动还原 `&amp;`/`&lt;`/`&gt;` 等 XML 转义字符，下游显示更准确

### 修复

- 修复 `雷神adb操作.枚举安卓控件()` 与 `寻找安卓控件()` 的 boolean 属性判断错误：此前 `clickable="false"`、`enabled="false"` 等属性可能被误判为 true
- 修复 `雷神adb操作.枚举安卓控件()` 与 `寻找安卓控件()` 在控件标题或备注包含 `&` 等特殊字符时可能卡死的问题
- 修复 `MuMu管理器.取信息()`、`取信息_单项()`、`到信息()` 在新版 MuMu 模拟器上部分运行时字段缺失时返回空值的问题：当 `info` 命令不再返回 `主窗口句柄`/`渲染窗口句柄`/`进程ID`/`虚拟机进程ID`/`ADB域名`/`ADB端口`/`启动状态` 等字段时，现在会自动通过进程枚举与官方端口公式补充

---

## v1.75.0.0 （2026-05-09）

### 修复

- 修复 `MuMu管理器.置配置()` 配置值处理时未添加引号导致的问题，现在为配置值添加单引号包裹
- 修复 `MuMu管理器.取配置_单项()` 配置名中包含点字符时的转义问题，确保正确解析 JSON 路径
- 修复 `M_加载渲染模块()` 加载失败时的初始化状态处理
...
## v1.72.0.0 （2026-01-13）

### 新增

- 新增 `雷神adb操作.枚举用户()` 枚举当前安卓系统中的所有用户。
- 新增 `雷神adb操作.创建用户()` 创建一个新用户。
- 新增 `雷神adb操作.删除用户()` 删除一个用户。
- 新增 `雷神adb操作.启动用户()` 启动一个未运行的用户。
- 新增 `雷神adb操作.切换用户()` 在已运行的用户间切换
- 新增 `雷神adb操作.停止用户()` 停止一个用户 (后台关闭其进程)。
- 新增 `雷神adb操作.取当前用户()` 获取当前前台用户ID。
- 新增 `雷神adb操作.取应用主类名()` 获取Android设备中指定包名的应用的主类名（启动类名）信息。
- 新增 `雷神adb操作.应用分身_创建()` 在用户空间中安装指定应用分身。
- 新增 `雷神adb操作.应用分身_删除()` 删除用户空间内的单个应用分身。
- 新增 `雷神adb操作.应用分身_启动()` 启动用户空间内的指定应用分身。
- 新增 `雷神adb操作.应用分身_停止()` 停止指定用户的单个应用分身。
- 新增 `雷神adb操作.应用分身_清除数据()` 强制停止应用分身并同时清除其所有数据，将应用恢复为首次安装时的状态。

### 优化

- 优化 `三个操作类.重载文件系统()` 现在可以将指定的目录重新挂载为读写、只读模式。
- 优化 `雷神adb操作.截屏_字节集()` 新增参数 `修复换行`，ADB（特别是旧版本）在传输字节集数据时，可能会对换行符进行转换，如果遇到这种情况，则需要进行修复才可以让图片正常。

### 修复

- 修复 `三个操作类.文件_是否存在_模拟器()` 无法正确判断的问题。
- 修复 `MuMu管理器.取信息_单项()` 当 索引 参数为空时，获取失败的问题。
- 修复 `取DOS返回_字节集()` 当返回内容只有1个字节时，会直接返回空字节的问题。
...
## v1.70.0.0 （2025-12-07）

### 新增

- 新增 `雷神adb操作.安装XAPK()` 将XAPK文件安装到模拟器。

### 优化

- 优化 `雷神_取APK信息()` 优化解析逻辑，确保能够更加准确的解析出需要的信息。原来需要提供多个参数来接受返回内容，现在只需要一个自定义类型的参数
- 优化 `雷神_取安装目录()` 增加MuMu模拟器支持，优化目录获取逻辑
- 优化 `雷神_取安装目录_全部()` 增加MuMu模拟器支持，并且避免重复添加相同安装目录。

---

## v1.69.0.0 （2025-11-29）

### 优化

- 优化 `三个操作类.文件_是否存在_模拟器()` 增加参数 `类型`，指定要判断的类型。1=文件或目录，2=文件，3=目录，4=大小大于0
- 优化 `雷神adb操作.取屏幕控件()` 优化代码，不在模拟器中创建临时文件。增加参数 `原始数据`，返回XML字节集内容。

### 修复

- 修复 `雷神adb操作.枚举目录文件()` 无法成功枚举出MuMu模拟器中的目录文件的问题。
- 修复 `雷神_设置窗口位置和尺寸()` 中的一个数据类型错误

---

## v1.68.0.0 （2025-11-05）

### 优化

- 优化 `雷神lsconsole操作`中的`取模拟器信息()`和`取模拟器信息EX()`，重写了这两个子程序，增加参数`匹配方式`并优化处理逻辑
- 优化 `zyJson` 更新到 v4.1.9

### 修复

- 修复 `M_取窗口句柄()` 当参数`主进程ID`和`绑定窗口句柄`时，不会返回主窗口句柄的问题。

---

## v1.67.0.0 （2025-11-01）

### 优化

- 优化 `雷神adb操作.自定义服务_安装()`、`雷神adb操作.自定义服务_启动()` 的代码逻辑

### 修复

- 修复 `雷神adb操作.截屏()` 可以正常截屏，并且功能变更为将图片保存到本地电脑中
- 修复 `雷神adb操作.截屏_字节集()` 可以正常截屏
...
## v1.65.0.0 （2025-10-17）

### 移除

### 新增

- 新增 `置默认终端()` 设置模块中执行DOS命令的默认终端。请确保终端可用性。
- 新增 `取默认终端()` 返回模块中执行DOS命令的默认终端。

### 优化

- 优化 模块中默认的执行终端，更换为 `PowerShell.exe`
- 优化 部分自定义命令中增加了参数`执行终端`，用于临时切换不同的终端执行命令。

### 修复

- 修复 `雷神adb操作.取屏幕控件()` 无法获取MuMu模拟器的屏幕控件的问题。

---

## v1.64.0.0 （2025-10-14）

### 移除

### 新增

- 新增 `M_取运行中目录()` 获取当前运行中的MuMu模拟器的工作目录。

### 优化

- 更名 `MuMu管理器.取shell目录()` 更名为 `MuMu管理器.取工作目录()`。
- 更名 `MuMu管理器.置shell目录()` 更名为 `MuMu管理器.置工作目录()`。
- 优化 `M_取系统配置目录`、`M_取安装目录()`、`M_取工作目录()`  
        增加参数`模拟器版本`指定需要获取的模拟器版本，可选常量：#版本_自动获取、#版本_Mu5、#版本_Mu5_海外版、#版本_Mu  
        支持获取海外版的目录，并且优先获取正在运行中的模拟器目录。  

### 修复

---

## v1.63.0.0 （2025-10-13）
...
- 优化 `雷神adb操作.设置代理()` 优化设置代理的方式。

### 修复

- 修复 `雷神_取安装目录()` 中的一个逻辑类型错误。
- 修复 `取DOS返回()`、`取DOS返回_字节集()` 当执行终端为 `powershell.exe`时，出现126错误的问题。
- 修复 `MuMu管理器.输入_U()` 部分表情符号被输入成问号的问题。

---

## v1.62.0.0 （2025-09-22）

### 移除

### 新增

- 新增 `MuMu管理器.shell()` 允许你直接向 MuMu模拟器 发送 ADB Shell 命令，而无需手动配置 ADB 连接或指定端口。
- 新增 `M_取工作目录()` 获取 MuMu模拟器 的工作目录。

### 优化

- 优化 `雷神adb操作.shell()` 当目标模拟器为 MuMu模拟器 时，自动使用新的方式执行。

### 修复

- 修复 `雷神adb操作` 当目标模拟器为 雷电模拟器 时，无法正常执行的问题。

---

## v1.61.0.0 （2025-08-20）
...
- 修复 `MuMu管理器` 索引为空时，被错误的当做 0 的问题。
- 修复 `雷神lsconsole操作` 部分函数导致意外崩溃的问题。
- 修复 `雷神adb操作` 部分情况下获取端口失败的问题。
...
## v1.59.0.1 （2025-06-24）

### 新增

### 优化

- 优化 `MuMu专用` 程序集适配兼容 MuM模拟器 5.0 。

### 修复

- 修复 内部哈希表清空后，未重新初始化，导致软件崩溃的问题。

---

## v1.59.0.0 （2025-06-24）

### 新增

- 新增 支持 `MuMu模拟器` 5.0 版本。  

### 优化

### 修复

- 修复 `MuMu管理器.启动()` 参数"启动应用"无法正确生效的问题。
- 修复 `雷神_按键设置()` 数据类型错误的问题。
...
## v1.57.0.0 （2025-05-27）

### 新增

### 优化

- 优化 `MuMu管理器.增强_截图()` 提高截图效率，去掉了输出32位图片的功能。
...
- 优化 `MuMu管理器` 中的 `索引` 参数，更改为 `通用型` ，可传递 `整数型` 和 `文本型`。
当类型为 `整数型` 时，索引值最好不要大于 65535 。

### 修复

---

## v1.54.0.0 （2025-05-09）
...
## v1.53.0.1 （2025-03-24）

### 新增

### 优化

- 优化 `MuMu管理器.增强_按键按下()` `MuMu管理器.增强_按键弹起()` 中的参数 `键值代码`  
更改为文本型，便于直接使用模块的 #安卓键\_\* 常量。
...
- 增加部分 MuMu 相关常量：#信息字段*\*、#工具栏**、#快捷\_*、#机型\_\*
...
- `CE服务端_启动()` 启动 CE 服务端（Cheat Engine Server），用于 Cheat Engine 通过网络连接到模拟器。  
模拟器需要使用桥接网络，并且开启 root。
...
- `三个操作类：安装sunny证书()` 安装 sunny 中间件的 cer 证书。
安装前请开启模拟器设置中的“root 权限”，雷电 9 还需要设置“System.vmdk 可写入”。
...
- 优化 `三个操作类.复制文件()` 增加了一个可选参数[附加参数]
  - 可以附加一些 cp 命令的额外参数。比如：“-f”强行复制文件或目录，不论目的文件或目录是否已经存在；“-r”递归处理，将指定目录下的文件与子目录一并处理。
  若源文件或目录的形态，不属于目录或符号链接，则一律视为普通文件处理；也可以是一个多参数组合，比如“-f -r”。
...
- `三个操作类.取应用版本()`
- 初步兼容 “MuMu 模拟器 12” 的 adb 连接，只是做了简单的兼容，因为内部结构与雷电模拟器差异较大，可能会有部分命令无法正常工作。
只兼容“MuMu 模拟器 12”，不支持“MuMu 模拟器 6”与“MuMu 模拟器 X”
...
- 修复上一个版本中“雷神\_取当前版本()”无法正确获取模拟器版本号，而出现的诸多问题。
  ###优化
- 优化 “三个操作类中的：枚举安卓控件()”，“寻找安卓控件()”，增加参数：“屏幕控件”，用来手动选择屏幕控件 xml 文本进行解析，
如果本参数留空，则自动执行“取屏幕控件”获取 xml。同时也更新了例程中的控件部分演示代码。
...
- 优化 `lsconsole.备份模拟器()`，移除参数"超时时间"；增加参数"是否压缩[逻辑型]"，为真时，备份文件会以 zip 格式进行压缩。
留空则不压缩。新参数需要雷神 1.0.9 以上版本支持。
